...
class LoadAndFind:
    ...
    def __init__(self):
        self.ft_data = pd.read_csv('ft50.trf', sep='\s+', header=None)  # data ft50.trf
        self.frequencies = self.ft_data[0]  # list axis x in data
        self.ppt = list(self.ft_data[1])  # list axis y in data

        self.peaks = find_peaks(self.ppt, height=Calculate_the_noise_level())  # finding peaks
        self.height = self.peaks[1]['peak_heights']  # list of the heights of the peaks
        self.peak_position = self.frequencies[self.peaks[0]]  # list of the peaks position
        ...
        self.export_data_periods = pd.DataFrame({'periods': self.periods, 'ppt': self.ppt})  # export data in periods
...
class SplitData(LoadAndFind):
    ...
    def __init__(self):
        super().__init__()
        self.parts_frequencies = np.array_split(self.frequencies, 25)  # split data in frequencies
...
class DistanceAndHistograms(LoadAndFind):
    ...
    def fitting_gauss(self, data, start_interval, end_interval, start_point_fit, end_point_fit, bins=50):
        ...
        self.H, self.A, self.x0, self.sigma = fc.Function.gauss_fit(self.x, self.y)  # gauss parameters
...
class Confirm(LoadAndFind):
    ...
    def run_fourier_transformation(self):
        ...
        self.export_1.to_csv('data_sorted.trf', sep='\t', header=None, index=None)
